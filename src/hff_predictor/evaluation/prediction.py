...
def prediction_performance_evaluation(Y_true, Y_pred, Y_pred_mod, Y_pred_non_mod):
    ...
    Y_nmod_pred_tot = pd.DataFrame(
        columns=["ynm_pred_isum", "ynm_true_sum", "ynm_error_isum", "ynm_perror_isum"],
        index=Y_pred.index,
    )
    ...
    for dt in Y_pred.index:
        ...
        Y_mod_pred_tot.loc[dt, "ym_error_isum"] = int(_yp_mod.sum()) - int(
            _yt_mod.sum()
        )
        Y_mod_pred_tot.loc[dt, "ym_perror_isum"] = int(
            int(_yp_mod.sum()) - int(_yt_mod.sum())
        ) / int(_yt_mod.sum())
        Y_mod_pred_tot.loc[dt, "ym_error_psum"] = _yp_mod_psum - int(_yt_mod.sum())
        Y_mod_pred_tot.loc[dt, "ym_perror_psum"] = int(
            _yp_mod_psum - int(_yt_mod.sum())
        ) / int(_yt_mod.sum())

        Y_nmod_pred_tot.loc[dt, "ynm_pred_isum"] = int(_yp_nmod.sum())
        Y_nmod_pred_tot.loc[dt, "ynm_true_sum"] = int(_yt_nmod.sum())
        Y_nmod_pred_tot.loc[dt, "ynm_error_isum"] = int(
            int(_yp_nmod.sum()) - int(_yt_nmod.sum())
        ) / int(_yt_nmod.sum())
    ...
    all_evaluations = {
        "p_modelable_products_total": Y_mod_pred_tot,
    ...
    mod_isum_err = abs(
        all_evaluations["p_modelable_products_total"]["ym_perror_isum"]
    ).mean()
    nmod_isum_err = abs(
        all_evaluations["p_nonmodelable_products_total"]["ynm_error_isum"]
    ).mean()
    mod_psum_err = abs(
        all_evaluations["p_modelable_products_total"]["ym_perror_psum"]
    ).mean()

    m_graph_title = "Foutmarges modelleerbaar: p-som: {}; i-som: {}".format(
        mod_psum_err, mod_isum_err
    )
    _gmod = all_evaluations["p_modelable_products_total"][
        [
            "ym_pred_isum",
            "ym_pred_psum",
            "ym_true_sum",
    ...
    _gnmod = all_evaluations["p_nonmodelable_products_total"][
        ["ynm_pred_isum", "ynm_true_sum", "ynm_error_isum"]
    ]
...
def in_sample_evaluation(pct_fits):
    ...
    print(
        "Overall median: {}; overall 5p: {}; overall 95p: {}".format(
            round(eval_time["median"].median(), 3),
    ...
    print(
        "Overall median: {}; overall 5p: {}; overall 95p: {}".format(
            round(eval_prod["median"].median(), 3),
...
def init_evaluate_prediction():
    active_products_act = hff_predictor.generic.files.import_temp_file(
        file_name="actieve_halffabricaten_wk_2020926_1610.csv", data_loc=fm.SAVE_LOC
    )
    inactive_products_act = hff_predictor.generic.files.import_temp_file(
        file_name="inactieve_halffabricaten_wk_2020926_1610.csv", data_loc=fm.SAVE_LOC
    )
