...
from hff_predictor.predict.make import run_prediction
...
def two_step_prediction(final_prediction_date):
    ...
    if type(final_prediction_date) == str:
        final_prediction_date = datetime.datetime.strptime(
            final_prediction_date, "%Y-%m-%d"
        )

    first_prediction_date = final_prediction_date - datetime.timedelta(days=7)
    __, pred1_diff, __, pred1 = run_prediction(
        pred_date=first_prediction_date, prediction_window=1
    )
    ...
    __, pred2_diff, __, __ = run_prediction(
        pred_date=first_prediction_date, prediction_window=2
    )
    ...
    return (
        pred1_combined["pred1_final"].rename(first_prediction_date),
...
def batch_2step_prediction(prediction_dates):
    ...
    prediction_dates = pd.DataFrame(
        pd.date_range("2020-07-01", periods=9, freq="W-MON").astype(str),
        columns=[cn.FIRST_DOW],
    )
    all_1step, all_2step = batch_2step_prediction(
        prediction_dates=prediction_dates[cn.FIRST_DOW]
    )
    ...
    hff_predictor.generic.files.save_to_csv(
        all_1step, file_name="1step_predictions", folder=fm.SAVE_LOC
    )
    hff_predictor.generic.files.save_to_csv(
        all_2step, file_name="2step_predictions", folder=fm.SAVE_LOC
    )
