...
def run_prediction(
    ...
    fit_data["avg_fit_error"], fit_data["avg_pct_fit_error"] = in_sample_error(
        all_fits=in_sample_fit, all_true_values=fit_data["y_true"]
    )
...
def batch_prediction(prediction_dates, model_settings):
    ...
    for p_date in prediction_dates[cn.FIRST_DOW]:
        ...
        all_is_abs_errors = pd.concat(
            [all_is_abs_errors, _fitdata["avg_fit_error"]], axis=0
        )
        all_is_pct_errors = pd.concat(
            [all_is_pct_errors, _fitdata["avg_pct_fit_error"]], axis=0
        )
...
def init_predict():
    ...
    is_fit1, os_pr1, fit_data1, predict_data1 = run_prediction(
        date_to_predict="2020-08-10",
        prediction_window=1,
        train_obs=cn.TRAIN_OBS,
        difference=False,
        lags=4,
    ...
    is_performance1 = in_sample_plot(y_true=fit_data1, y_fit=is_fit1, title="test")
    ...
    test2 = run_prediction_bootstrap(
        date_to_predict="2020-09-28",
        prediction_window=2,
        train_obs=cn.TRAIN_OBS,
        difference=False,
        lags=cn.N_LAGS,
