...
def create_model_setup(
    ...
    def create_predictive_context(mod, non_mod, exog_f, hold_out=hold_out):
        ...
        return (
            mod.shift(-hold_out)[:-hold_out],
            non_mod.shift(-hold_out)[:-hold_out],
...
def init_setup_prediction():
    active_products_t = hff_predictor.generic.files.import_temp_file(
        file_name=fm.ORDER_DATA_ACT, data_loc=fm.SAVE_LOC, set_index=True
    )

    inactive_products_t = hff_predictor.generic.files.import_temp_file(
        file_name=fm.ORDER_DATA_INACT, data_loc=fm.SAVE_LOC, set_index=True
    )

    exog_features_t = hff_predictor.generic.files.import_temp_file(
        file_name=fm.EXOG_FEATURES, data_loc=fm.SAVE_LOC, set_index=True
    )
