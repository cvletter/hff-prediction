...
if __name__ == "__main__":
    ...
    inactive_products_act = import_temp_file(
    ...
    def out_of_sample_performance(
        all_predictions, all_true_values, all_mod_prod, all_non_mod_prod, zero_pred=True
    ):
        ...
        all_prediction_dates = predictions_total.index.unique().sort_values(
            ascending=False
        )
        ...
        for d in all_prediction_dates:
            ...
            _pred_err_mod_sum = round(
                abs(_pred_mod_sum - _truev_mod_sum) / _truev_mod_sum, 3
            )
            _pred_err_nmod_sum = round(
                abs(_pred_nmod_sum - _truev_nmod_sum) / _truev_nmod_sum, 3
            )
            _pred_err_tot = round(abs(_pred_tot - _truev_tot) / _truev_tot, 3)

            _predq_err_mod = round(
                abs(_pred_mod_med - _truev_mod_sum) / _truev_mod_sum, 3
            )
            _predq_err_nmod = round(
                abs(_pred_nmod_med - _truev_nmod_sum) / _truev_nmod_sum, 3
            )
    ...
    pred_t, pred_m, pred_nm, r_predmod, r_truemod = out_of_sample_performance(
    ...
    print(
        "Average pct error, total: {}, modelable: {}, non-modelable: {}".format(
    ...
    print(
        "Average pct error without peak-period, total: {}, modelable: {}, non-modelable: {}".format(
    ...
    def plot_results(results):
        ...
        pct_error = round(
            (
                abs(plot_data["prediction"] - plot_data["true_value"])
