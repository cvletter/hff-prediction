...
def prep_weather_features(
    ...
    weather_combined = weather_current_week.join(weather_min_1w, how="left").join(
...
def prep_holiday_features(
    weekly=False, shift=True, prediction_window=cn.PREDICTION_WINDOW
):
    ...
    holiday_dates["pre_christmas"] = [
        1 if x in pre_christmas_dt else 0 for x in holiday_dates["day"]
    ]
...
def prep_exogenous_features(
    ...
    weather_f = prep_weather_features(
        input_weer_data=weather_data_processed,
        prediction_window=prediction_window,
        shift=shift,
    )
    holiday_f = prep_holiday_features(
        weekly=True, prediction_window=prediction_window, shift=shift
    )
    covid_f = prep_covid_features(weekly=True)
    level_f = prep_level_shifts()

    exog_features = (
        weather_f.join(holiday_f, how="left")
        .join(covid_f, how="left")
...
if __name__ == "__main__":
    ...
    exog_features = prep_exogenous_features(
    ...
    def get_correlations(dep_var, indep_var):
        ...
        corr_stat = pd.DataFrame(
            index=corrs.index, columns=["avg", "25p", "med", "75p"]
        )
